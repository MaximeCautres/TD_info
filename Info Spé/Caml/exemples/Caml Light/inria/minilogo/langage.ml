...
let rec exécute_ordre env = function
  ...
  | Ve -> vide_écran()
  | Rep (n, l) ->
     for i = 1 to valeur_entière (valeur_expr env n) do
       do_list (exécute_ordre env) l
     done
  ...
  | Exécute (nom_de_procédure, args) ->
     ...
     let rec augmente_env = function
       ...
       | _ -> failwith ("mauvais nombre d'arguments pour "
                    ^ nom_de_procédure) in     let env_pour_corps = augmente_env (variables, args) in
     try
       do_list (exécute_ordre env_pour_corps) corps
