          ...
          if l1 = 0 then raise Erreur
            ...
            begin
              ...
              let l2 = list_length t.(i).(k) in
                if l2 = 1 && l1 <> l2 then réduis_aux t i k (hd t.(i).(k));
            end;
      end;
    if k <> i then
      begin
          ...
          if l1 = 0 then raise Erreur
            ...
            begin
              ...
              let l2 = list_length t.(k).(j) in
                if l2 = 1 && l1 <> l2 then réduis_aux t k j (hd t.(k).(j));
            end;
      end;
  done;
              ...
              if l1 = 0 then raise Erreur
                ...
                begin
                  ...
                  let l2 = list_length t.(u).(v) in
                    if l2 = 1 && l1 <> l2 then réduis_aux t u v (hd t.(u).(v));
                end;
          end;
      done;
    done;
...
let prepare t =
  for i = 0 to 8 do
    for j = 0 to 8 do
      if list_length t.(i).(j) = 1 then réduis_aux t i j (hd t.(i).(j));
    done;
  done;
;;
...
let traite fichier =
  let t1 = sys__time() in
    let canalin = open_in fichier in
      ...
      with End_of_file ->
            close_in canalin;
            print_string "durée: "; print_float (sys__time() -. t1); print_string " s";
            print_newline()
;;
