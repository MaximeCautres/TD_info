				...
				let boutons = make_vect n (bouton "" 0 0 0 0 0 (fun () -> ())) in
					for i = 0 to n - 1 do
						boutons.(i) <- {titre = titres.(i); orx = i * 34; ory = 0; hauteur = 30;
							largeur = 34; couleur = Graphics.white; action = fun () -> ()}
					done;
					set_actions boutons;
...
let boucle1 cube action1 action2 action3 =
	Graphics.open_graph " 612x612";
	try
		cube.dessine1 ();
		cree_boutons1 cube;
		let largeur, hauteur = Graphics.text_size "Quitter"
		and largeur1, hauteur1 = Graphics.text_size "Mélanger"
		and largeur2, hauteur2 = Graphics.text_size "Résolution simple"
		and largeur3, hauteur3 = Graphics.text_size "Résolution complète"
		
		in
			let bouton_quitter = bouton "Quitter" (Graphics.size_x () - largeur - 20) (Graphics.size_y () - hauteur - 20)
				(largeur + 10) (hauteur + 10) Graphics.yellow (fun () -> raise Quitter)
			and
			bouton_melanger = bouton "Mélanger" ((Graphics.size_x () - largeur1) / 2 - 10) (Graphics.size_y () - hauteur1 - 20)
				(largeur1 + 10) (hauteur1 + 10) Graphics.yellow action1
			and
			bouton_resoudre1 = bouton "Résolution simple" 20 (Graphics.size_y () - hauteur2 - 20)
				(largeur2 + 10) (hauteur2 + 10) Graphics.yellow action2
			and
			bouton_resoudre2 = bouton "Résolution complète" 20 (Graphics.size_y () - hauteur3 - 60)
				(largeur3 + 10) (hauteur3 + 10) Graphics.yellow action3
			in
				dessine_bouton bouton_quitter;
				dessine_bouton bouton_melanger;
				dessine_bouton bouton_resoudre1;
				dessine_bouton bouton_resoudre2;
				dessine_boutons1 cube;
				while true do
					let status = Graphics.wait_next_event [Graphics.Button_down] in
						...
						in
							if status.Graphics.button then (
									...
									gestion_bouton bouton_resoudre1 mousex mousey;
									gestion_bouton bouton_resoudre2 mousex mousey;
...
let boucle2 cube action1 action2 action3 =
	Graphics.open_graph " 612x612";
	try
		cube.dessine2 ();
		cree_boutons2 cube;
