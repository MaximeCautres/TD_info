...
type état =   { mutable X : float; mutable Y : float;
...
let avance d =    let dx = d *. cos (crayon.Visée)
    and dy = d *. sin (crayon.Visée) in    crayon.X <- crayon.X +. dx;
    crayon.Y <- crayon.Y +. dy;    if crayon.Levé
